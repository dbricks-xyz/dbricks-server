...
import { SERUM_PROG_ID } from './config';
...
export function getMint(name: string) {
  ...
  if (targetMint.length === 0) {
    throw new Error(`Mint with name ${name} and network ${process.env.NETWORK} not found`);
  }
...
export function getSerumMarket(name: string) {
  ...
  if (targetMarket.length === 0) {
    throw new Error(`Market with name ${name} and network ${process.env.NETWORK} not found`);
  } else if (targetMarket[0].deprecated) {
    throw new Error(`Market with name ${name} and network ${process.env.NETWORK} is deprecated`);
  }
